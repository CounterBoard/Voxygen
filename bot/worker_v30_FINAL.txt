...
async function inviteResident(request, env) {
  ...
  const territory=await env.DB.prepare(`SELECT id,name,owner_telegram_id,owner_telegram_username,is_government FROM territories WHERE id=? AND status='approved'`).bind(territoryId).first();
...
async function uploadScreenshot(request, env) {
  const auth=await requireUser(request,env);if(auth.error)return auth;
  // is_government is a lazy-migrated column; ensure the existing D1 schema is current
  // before deciding whether the upload can bypass moderation.
  await ensureWorldTables(env);
