...
async function createCustomizationItem(request, env) {
  ...
  try{
    ...
    await cleanupCustomizationCategories(env);
    if(governmentMarker){
      await env.DB.prepare(`UPDATE customization_items SET government_marker=0 WHERE id<>?`).bind(itemId).run();
...
async function updateCustomizationItem(request,env,itemId){
  ...
  await cleanupCustomizationCategories(env);
  if(isMarker&&governmentMarker){
    await env.DB.prepare(`UPDATE customization_items SET government_marker=0 WHERE id<>?`).bind(id).run();
    await env.DB.prepare(`UPDATE city_customization SET marker_item_id=?,updated_at=? WHERE territory_id IN (SELECT id FROM territories WHERE status='approved' AND is_government=1)`).bind(id,Date.now()).run();
  }else if(isMarker&&!governmentMarker){
    await env.DB.prepare(`UPDATE city_customization SET marker_item_id=NULL,updated_at=? WHERE marker_item_id=? AND territory_id IN (SELECT id FROM territories WHERE status='approved' AND is_government=1)`).bind(Date.now(),id).run();
  }
  if(achievement.enabled) {
    const territories=await env.DB.prepare(`SELECT id FROM territories WHERE status='approved'`).all();
    for(const t of territories.results||[]) await evaluateAchievementsForTerritory(env,t.id);
