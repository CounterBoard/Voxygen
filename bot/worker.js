...
async function getTerritories(request, env) {
  try {
    ...
    const territories=(result.results||[]).map(row=>{
      ...
      return {...row,x,y,vox_x:parsed?.x??null,vox_z:parsed?.z??null,curator:weightedRows(inspectorRows,2),community:weightedRows(playerRows,5),votes:playerRows.length,my_rating:myRating,owner:row.owner_mc_nickname||'Ник не указан',owner_mc_nickname:row.owner_mc_nickname||null,founder_mc_nickname:row.owner_mc_nickname||null,managers,manager_telegram_ids:managerIds,manager_count:managers.length,is_manager:isManagerViewer,residents:ms.map(m=>m.mc_nickname).filter(Boolean),resident_telegram_ids:ms.map(m=>Number(m.telegram_id)),resident_count:ms.length+managers.length,recruitment_contact_username:row.recruitment_contact_username||row.owner_telegram_username||null,reviews:commentsByCity[row.id]||[],screenshots:screensByCity[row.id]||[],marker_url:row.marker_image||null};
    });
...
async function getMe(request, env) {
  ...
  for(const c of [...(founderRows.results||[]),...(managerRows.results||[])]) await evaluateAchievementsForTerritory(env,c.id).catch(error=>console.error('Achievement evaluation after getMe:',error));
...
async function uploadScreenshot(request, env) {
  ...
  const username=normalizeUsername(user.username)||`id${user.id}`,token=String(env.BOT_TOKEN||'').trim(),screenshotId=crypto.randomUUID(),status=(role==='inspector'||role==='main_inspector')?'approved':'pending';
  let tgData=null;
  if(status==='pending'){
...
async function deleteScreenshot(request, env, screenshotId) {
  const auth=await requireUser(request,env);if(auth.error)return auth;
  const row=await env.DB.prepare(`SELECT s.id,s.territory_id,t.owner_telegram_id FROM screenshots s JOIN territories t ON t.id=s.territory_id WHERE s.id=? AND s.status='approved'`).bind(screenshotId).first();
  if(!row)return json({ok:false,error:'Screenshot not found'},404);if(!(await isTerritoryManager(env,row.territory_id,auth.user.id)))return json({ok:false,error:'Только основатель/мэр может удалить скрин'},403);
...
async function saveRecruitment(request, env) {
  ...
  const requestedUsername=normalizeUsername(body?.recruitmentUsername)||''; let contactUsername=requestedUsername;
  if(contactUsername){const candidate=await env.DB.prepare(`SELECT telegram_id FROM users WHERE lower(replace(telegram_username,'@',''))=lower(?) LIMIT 1`).bind(contactUsername).first(); if(!candidate || !(await isTerritoryManager(env,territoryId,candidate.telegram_id))) return json({ok:false,error:'Этот username не относится к основателю/мэру города'},400);}
  if(!contactUsername){const managers=await getTerritoryManagerRows(env,territoryId); contactUsername=normalizeUsername(managers.find(m=>m.telegram_username)?.telegram_username)||'';}
