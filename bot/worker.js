...
async function ensureWorldTables(env) {
  ...
  // Remove the old built-in flags and all their city assignments. Flags are now created only by the main inspector.
  await env.DB.prepare(`DELETE FROM city_items WHERE item_id IN ('','','')`).run().catch(()=>{});
  await env.DB.prepare(`DELETE FROM user_items WHERE item_id IN ('','','')`).run().catch(()=>{});
  await env.DB.prepare(`DELETE FROM customization_items WHERE id IN ('','','')`).run().catch(()=>{});
  await env.DB.prepare(`UPDATE city_customization SET marker_item_id=NULL WHERE marker_item_id IN ('','','')`).run().catch(()=>{});
  await env.DB.prepare(`DELETE FROM customization_categories WHERE lower(name) IN ('за достижения','знамя') AND id IN ('achievement-category','flag-category')`).run().catch(()=>{});
...
async function evaluateAchievementsForTerritory(env, territoryId) {
  ...
  const visualsRow = await env.DB.prepare(`
    SELECT COUNT(*) AS value FROM city_items ci
...
async function deleteCustomizationItem(request,env,itemId){
  ...
  await env.DB.prepare(`DELETE FROM achievement_awards WHERE item_id=?`).bind(id).run().catch(()=>{});
...
async function purchaseCustomizationItem(request, env) {
  ...
  if(territoryId && Number(item.price)>0){
    await env.DB.prepare(`INSERT INTO gem_spend_log(id,territory_id,telegram_id,item_id,amount,created_at) VALUES(?,?,?,?,?,?)`)
      .bind(crypto.randomUUID(),territoryId,userId,itemId,Number(item.price),Date.now()).run();
