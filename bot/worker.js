...
async function getCustomization(request, env) {
  ...
  let requestedAccount='personal';
  try { const u=new URL(request.url); requestedAccount=String(u.searchParams.get('account')||'personal').toLowerCase()==='treasury'?'treasury':'personal'; } catch {}
  const accountScope=(requestedAccount==='treasury' && founderCity) ? 'treasury' : 'personal';
  if(accountScope==='treasury') await env.DB.prepare(`INSERT OR IGNORE INTO city_treasury(territory_id,balance,updated_at) VALUES(?,?,?)`).bind(founderCity.id,0,Date.now()).run();
  ...
  const [cats,items,owned,balance]=await Promise.all([
    ...
    accountScope==='treasury' ? env.DB.prepare('SELECT balance FROM city_treasury WHERE territory_id=?').bind(founderCity.id).first() : env.DB.prepare('SELECT balance FROM user_currency WHERE telegram_id=?').bind(userId).first(),
  ]);
  ...
  return json({ok:true,balance:Number(balance?.balance||0),personalBalance:Number((await env.DB.prepare('SELECT balance FROM user_currency WHERE telegram_id=?').bind(userId).first())?.balance||0),accountScope,accountLabel:accountScope==='treasury'?`Казна: ${founderCity?.name||'города'}`:(currentUser?.mc_nickname||'Ник не указан'),cityName:founderCity?.name||null,categories:cats.results||[],items:items.results||[],
...
async function purchaseCustomizationItem(request, env) {
  ...
  const requestedAccount=String(body?.accountScope||'personal').toLowerCase()==='treasury'?'treasury':'personal';
  const founderCity=await env.DB.prepare(`SELECT id,name FROM territories WHERE owner_telegram_id=? AND status='approved' LIMIT 1`).bind(userId).first();
  const accountScope=(requestedAccount==='treasury' && founderCity)?'treasury':'personal';
  if(accountScope==='treasury') await env.DB.prepare(`INSERT OR IGNORE INTO city_treasury(territory_id,balance,updated_at) VALUES(?,?,?)`).bind(founderCity.id,0,Date.now()).run();
  ...
  await env.DB.prepare('INSERT OR IGNORE INTO user_currency (telegram_id, balance) VALUES (?, 0)').bind(userId).run();
  const balance = accountScope==='treasury'
    ? await env.DB.prepare('SELECT balance FROM city_treasury WHERE territory_id = ?').bind(founderCity.id).first()
    : await env.DB.prepare('SELECT balance FROM user_currency WHERE telegram_id = ?').bind(userId).first();
  if (Number(balance?.balance || 0) < Number(item.price)) return json({ ok: false, error: 'Недостаточно самоцветов' }, 400);
  const charged = accountScope==='treasury'
    ? await env.DB.prepare('UPDATE city_treasury SET balance = balance - ?, updated_at = ? WHERE territory_id = ? AND balance >= ?').bind(Number(item.price),Date.now(),founderCity.id,Number(item.price)).run()
  ...
  return json({ ok: true, balance: Number(balance.balance) - Number(item.price), accountScope });
}

async function getTreasury(request, env) {
  const auth=await requireUser(request,env); if(auth.error)return auth;
  await ensureWorldTables(env);
  const userId=Number(auth.user.id),url=new URL(request.url),territoryId=String(url.searchParams.get('territoryId')||'').trim();
  ...
  const membership=await env.DB.prepare(`SELECT telegram_id FROM city_memberships WHERE telegram_id=? AND territory_id=?`).bind(userId,territoryId).first();
  const isOwner=String(territory.owner_telegram_id||'')===String(userId);
  if(!isOwner&&!membership)return json({ok:false,error:'Только жители города имеют доступ к казне'},403);
  ...
  return json({ok:true,balance:Number(treasury?.balance||0),personal_balance:Number(personal?.balance||0),isOwner,cityName:territory.name});
}
async function treasuryDeposit(request, env) {
  ...
  if(!territoryId||!Number.isSafeInteger(amount)||amount<=0)return json({ok:false,error:'Введите положительное количество самоцветов'},400);
  ...
  const membership=await env.DB.prepare(`SELECT telegram_id FROM city_memberships WHERE telegram_id=? AND territory_id=?`).bind(userId,territoryId).first(),isOwner=String(territory.owner_telegram_id||'')===String(userId); if(!isOwner&&!membership)return json({ok:false,error:'Только жители города могут пополнять казну'},403);
...
async function treasuryWithdraw(request, env) {
  ...
  const territory=await env.DB.prepare(`SELECT id,name,owner_telegram_id FROM territories WHERE id=? AND status='approved'`).bind(territoryId).first(); if(!territory)return json({ok:false,error:'City not found'},404); if(String(territory.owner_telegram_id||'')!==String(userId))return json({ok:false,error:'Только основатель/мэр может тратить средства казны'},403);
  await env.DB.prepare(`INSERT OR IGNORE INTO user_currency(telegram_id,balance) VALUES(?,0)`).bind(userId).run(); await env.DB.prepare(`INSERT OR IGNORE INTO city_treasury(territory_id,balance,updated_at) VALUES(?,?,?)`).bind(territoryId,0,Date.now()).run();
  const charged=await env.DB.prepare(`UPDATE city_treasury SET balance=balance-?,updated_at=? WHERE territory_id=? AND balance>=?`).bind(amount,Date.now(),territoryId,amount).run(); if(!charged.meta?.changes)return json({ok:false,error:'Недостаточно средств в казне'},400);
  await env.DB.prepare(`UPDATE user_currency SET balance=balance+? WHERE telegram_id=?`).bind(amount,userId).run(); const [treasury,personal]=await Promise.all([env.DB.prepare(`SELECT balance FROM city_treasury WHERE territory_id=?`).bind(territoryId).first(),env.DB.prepare(`SELECT balance FROM user_currency WHERE telegram_id=?`).bind(userId).first()]); return json({ok:true,balance:Number(treasury?.balance||0),personal_balance:Number(personal?.balance||0),isOwner:true});
}
...
async function getTerritories(request, env) {
  try {
    ...
    const [members, comments, screens, ratings, inspectorRatings] = await Promise.all([
    ...
    const byCity={},commentsByCity={},screensByCity={},scoresByCity={},inspectorScoresByCity={},viewerRatingsByCity={};
    ...
    const territories=(result.results||[]).map(row=>{
      ...
      return {...row,x,y,vox_x:parsed?.x??null,vox_z:parsed?.z??null,
        curator:weightedRows(inspectorRows,2),community:weightedRows(playerRows,5),votes:playerRows.length,my_rating:myRating,
...
async function getMe(request, env) {
  ...
  try {
    await env.DB.prepare(`UPDATE users SET telegram_username=? WHERE telegram_id=?`).bind(user.username || null, Number(user.id)).run();
  } catch (e) { console.error('Telegram username sync error:', e); }
  try {
...
async function requestInspectorRating(request, env) {
  ...
  const territory = await env.DB.prepare(`
    SELECT t.id, t.name, t.coords, t.owner_telegram_id, t.owner_telegram_username, t.owner_input,
           u.mc_nickname AS owner_mc_nickname
    FROM territories t
    LEFT JOIN users u ON u.telegram_id = t.owner_telegram_id
    WHERE t.id = ? AND t.status = 'approved'
  `).bind(territoryId).first();
  if (!territory) return json({ ok: false, error: 'City not found' }, 404);
  if (String(territory.owner_telegram_id || '') !== String(auth.user.id)) {
    return json({ ok: false, error: 'Только основатель/мэр может запросить оценку инспектора' }, 403);
  }

  const rows = await env.DB.prepare('SELECT telegram_id FROM inspectors').all();
  const recipients = new Set();
  const mainChatId = String(env.INSPECTOR_CHAT_ID || '').trim();
  if (mainChatId) recipients.add(mainChatId);
  for (const row of rows.results || []) {
    const id = String(row.telegram_id || '').trim();
    if (id) recipients.add(id);
  }
  if (!recipients.size) return json({ ok: false, error: 'Инспекторы не настроены' }, 500);

  const cityName = String(territory.name || 'Без названия');
  const ownerUsername = normalizeUsername(territory.owner_telegram_username || auth.user.username) || 'username не указан';
  const parsedCoords = parseRequestCoords(territory.coords);
  const coords = parsedCoords ? `${parsedCoords.x} ${parsedCoords.z}` : String(territory.coords || 'Координаты не указаны');
  const text =
    `⭐ <b>Запрос на оценку города</b>
...
` +
  ...
  for (const chatId of recipients) {
    try {
      const result = await telegram(env, 'sendMessage', { chat_id: chatId, text, parse_mode: 'HTML' });
      if (result.response.ok && result.data?.ok) sent++;
      else { failed++; console.error('Inspector rating request Telegram error:', chatId, result.data); }
    } catch (error) {
      failed++;
      console.error('Inspector rating request error:', chatId, error);
    }
  }
  if (!sent) return json({ ok: false, error: 'Не удалось отправить запрос инспекторам', sent, failed }, 502);
...
async function saveRating(request, env) {
  ...
  if(founderId&&(curatorChanged||communityChanged)){
    try{
      const changes=[];
      ...
      await telegram(env,'sendMessage',{chat_id:founderId,text:`⭐ Оценка вашего города изменилась\n\n🏙 ${escapeHtml(territory.name)}\n${changes.join('\n')}`,parse_mode:'HTML'});
    }catch(error){console.error('Founder rating notification error:',error)}
  }
...
async function addComment(request, env) {
  ...
  if(founderId&&String(founderId)!==String(auth.user.id)){
    try{await telegram(env,'sendMessage',{chat_id:founderId,text:`💬 Новый комментарий\n\n🏙 ${escapeHtml(territory.name)}\n👤 ${escapeHtml(username)}\n\n💬 ${escapeHtml(text)}`,parse_mode:'HTML'});}catch(error){console.error('Founder comment notification error:',error)}
  }
...
async function replyToComment(request, env) {
  ...
  if(String(c.owner_telegram_id||'')!==String(auth.user.id))return json({ok:false,error:'Недостаточно прав'},403);
...
async function updateReply(request, env) {
  ...
  const row=await env.DB.prepare(`SELECT t.owner_telegram_id FROM comment_replies cr JOIN comments c ON c.id=cr.comment_id JOIN territories t ON t.id=c.territory_id WHERE cr.comment_id=?`).bind(commentId).first();
  if(!row)return json({ok:false,error:'Ответ не найден'},404);if(String(row.owner_telegram_id||'')!==String(auth.user.id))return json({ok:false,error:'Недостаточно прав'},403);
...
async function getRecruitment(env) {
  try {
    ...
    const result = await env.DB.prepare(`
      SELECT t.id,t.name,t.coords,t.owner_telegram_id,t.owner_telegram_username,t.owner_input,
             COALESCE(r.description,'') AS recruitment_description,
             COALESCE(r.enabled,0) AS recruitment_enabled,
             COALESCE(ou.mc_nickname,t.owner_input,'Ник не указан') AS owner_mc_nickname,
             t.created_at
      FROM territories t
      LEFT JOIN recruitment r ON r.territory_id=t.id
      LEFT JOIN users ou ON ou.telegram_id=t.owner_telegram_id
      WHERE t.status='approved' AND COALESCE(r.enabled,0)=1 AND TRIM(COALESCE(r.description,''))<>''
      ORDER BY t.created_at DESC
    `).all();
    const rows=result.results||[];
    for(const row of rows){const score=await calculateTerritoryScores(env,row.id);row.curator=score.curator;row.community=score.community;row.votes=score.votes;}
    rows.sort((a,b)=>Number(b.curator||0)-Number(a.curator||0)||Number(b.created_at||0)-Number(a.created_at||0));
    return json({ok:true,cities:rows});
  } catch(error){console.error('Recruitment list error:',error);return json({ok:false,error:'Database error'},500)}
}
