...
async function requestInspectorRating(request, env) {
  ...
  const ownerName = String(territory.owner_mc_nickname || territory.owner_input || territory.owner_telegram_username || 'Основатель/мэр');
  const coords = String(territory.coords || 'Координаты не указаны');
  const text =
    `⭐ <b>Запрос на оценку города</b>\n\n` +
    `🏙 <b>${escapeHtml(cityName)}</b>\n` +
    `👤 Основатель/мэр: ${escapeHtml(ownerName)}\n` +
    `📍 <code>${escapeHtml(coords)}</code>\n\n` +
    `Основатель/мэр просит инспекторов оценить этот город в Voxygen.`;
