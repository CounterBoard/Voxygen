...
// Schema initialization is lazy for existing D1 databases, but must not run
// on every polling request. The Mini App refreshes territories every 15s and
// the Vox grid every 5s; repeated DDL/PRAGMA/cleanup work can cause D1
// contention and make Telegram report a generic "Failed to fetch".
let worldTablesReady = null;
let recruitmentTableReady = null;

async function ensureWorldTablesOnce(env) {
  if (!worldTablesReady) {
    worldTablesReady = ensureWorldTables(env).catch((error) => {
      worldTablesReady = null;
      throw error;
    });
  }
  return worldTablesReady;
}

async function ensureRecruitmentTableOnce(env) {
  if (!recruitmentTableReady) {
    recruitmentTableReady = ensureRecruitmentTable(env).catch((error) => {
      recruitmentTableReady = null;
      throw error;
    });
  }
  return recruitmentTableReady;
}
...
async function ensureWorldTables(env) {
  ...
  const customizationItemCols = await env.DB.prepare(`PRAGMA table_info(customization_items)`).all();
  const customizationItemNames = new Set((customizationItemCols.results || []).map(r => r.name));
  if (!customizationItemNames.has('government_marker')) { try { await env.DB.prepare(`ALTER TABLE customization_items ADD COLUMN government_marker INTEGER NOT NULL DEFAULT 0`).run(); } catch (e) { if (!String(e).toLowerCase().includes('duplicate column')) throw e; } }
  ...
  `).run();
  const cityCustomizationCols = await env.DB.prepare(`PRAGMA table_info(city_customization)`).all();
  const cityCustomizationNames = new Set((cityCustomizationCols.results || []).map(r => r.name));
  if (!cityCustomizationNames.has('background_item_id')) { try { await env.DB.prepare(`ALTER TABLE city_customization ADD COLUMN background_item_id TEXT`).run(); } catch (e) { if (!String(e).toLowerCase().includes('duplicate column')) throw e; } }
  await env.DB.prepare(`
...
async function ensureFirstCityMarker(env, territoryId) {
  ...
  // First, preserve a currently equipped active marker. If it is gone/inactive,
  // clear the stale reference so the first available marker can be restored.
  const current=await env.DB.prepare(`SELECT marker_item_id,background_item_id FROM city_customization WHERE territory_id=?`).bind(id).first();
  if(current?.marker_item_id){
    const active=await env.DB.prepare(`
      SELECT i.id,i.name,i.image_data
      FROM customization_items i
      LEFT JOIN customization_categories c ON c.id=i.category_id
      WHERE i.id=? AND i.active=1
        AND (COALESCE(c.name,'') IN ('Метка','метка','МЕТКА')
          OR instr(',' || replace(replace(replace(replace(COALESCE(i.categories_text,''),' ',''),'Метка','метка'),'МЕТКА','метка'),'МЕтка','метка') || ',', ',метка,') > 0)
    `).bind(current.marker_item_id).first();
  ...
  // Build the city's visual inventory from city-earned visuals plus visuals
  // owned by the current founders/mayors and residents. The earliest acquired
  // marker is the one that must be equipped automatically.
  ...
  const ids=[...new Set([...managerIds,...(members.results||[]).map(m=>Number(m.telegram_id)).filter(Boolean)])];
  const ph=ids.length?ids.map(()=>'?').join(','):'NULL';

  const first=await env.DB.prepare(`
    SELECT item_id,name,image_data,acquired_at,sort_order FROM (
      SELECT ci.item_id,i.name,i.image_data,ci.acquired_at,i.sort_order
      FROM city_items ci
      JOIN customization_items i ON i.id=ci.item_id
      LEFT JOIN customization_categories c ON c.id=i.category_id
      ...
      LEFT JOIN customization_categories c ON c.id=i.category_id
      WHERE ui.telegram_id IN (${ph}) AND i.active=1
        AND (COALESCE(c.name,'') IN ('Метка','метка','МЕТКА')
          OR instr(',' || replace(replace(replace(replace(COALESCE(i.categories_text,''),' ',''),'Метка','метка'),'МЕТКА','метка'),'МЕтка','метка') || ',', ',метка,') > 0)
    ) inventory
...
async function getVoxGrid(env) {
  await ensureWorldTablesOnce(env);
  const rows = await env.DB.prepare(
...
async function evaluateAchievementsForTerritory(env, territoryId) {
  ...
  for (const item of items.results || []) {
    ...
    // Keep the reward row self-healing even when the achievement award already
    // exists from an earlier request. This also repairs older cities where the
    // achievement was recorded but the city visual row was not persisted.
    await env.DB.prepare(`
      INSERT OR IGNORE INTO city_items(territory_id,item_id,acquired_at,source)
      VALUES(?,?,?,'achievement')
    `).bind(territoryId,item.id,Date.now()).run();

    // IMPORTANT: do this for every achieved visual, not only when the
    // achievement_awards INSERT reports changes. D1 requests can revisit an
    // already-awarded achievement, and the city must still receive/equip its
    // first marker. The helper itself keeps an existing valid marker untouched.
...
async function getCustomization(request, env) {
  ...
  const [cats,items,owned,personalBalance]=await Promise.all([
    ...
    env.DB.prepare(`SELECT i.id,i.category_id,c.name AS category_name,i.name,i.price,i.image_data,i.categories_text,i.color,i.active,i.sort_order,i.achievement_enabled,i.achievement_residents,i.achievement_gems,i.achievement_inspector_score,i.achievement_player_score,i.achievement_screenshots,i.achievement_comments,i.achievement_visuals,i.government_marker FROM customization_items i LEFT JOIN customization_categories c ON c.id=i.category_id WHERE i.active=1 ORDER BY i.sort_order ASC, i.created_at ASC`).all(),
  ...
  const viewerRole=await getRoleByTelegramId(env,userId,auth.user.username);
  const visibleItems=viewerRole==='main_inspector' ? (items.results||[]) : (items.results||[]).filter(i=>Number(i.government_marker||0)!==1);
  const itemMap=new Map(visibleItems.map(i=>[i.id,i]));
  const myItems=(owned.results||[]).map(x=>({...x,...(itemMap.get(x.item_id)||{})})).filter(x=>x.item_id&&itemMap.has(x.item_id));
  let cityInventory=[],equippedMarkerItemId=null,equippedBackgroundItemId=null;
  if(profileTerritoryId){
    const city=await env.DB.prepare('SELECT marker_item_id,background_item_id FROM city_customization WHERE territory_id=?').bind(profileTerritoryId).first();
    equippedMarkerItemId=city?.marker_item_id||null;
    equippedBackgroundItemId=city?.background_item_id||null;
    ...
    const [cityOnly,userOwned]=await Promise.all([
      env.DB.prepare(`SELECT ci.territory_id,ci.item_id,i.name,i.price,i.image_data,c.name AS category_name,i.categories_text,i.color,i.government_marker,ci.source FROM city_items ci JOIN customization_items i ON i.id=ci.item_id LEFT JOIN customization_categories c ON c.id=i.category_id WHERE ci.territory_id=?`).bind(profileTerritoryId).all(),
      env.DB.prepare(`SELECT ui.telegram_id,ui.item_id,i.name,i.price,i.image_data,c.name AS category_name,i.categories_text,i.color,i.government_marker,ui.source FROM user_items ui JOIN customization_items i ON i.id=ui.item_id LEFT JOIN customization_categories c ON c.id=i.category_id WHERE ui.telegram_id IN (${ph})`).bind(...ids).all(),
    ]);
...
function isCoverTag(categories){return categories.some(x=>String(x).trim().toLowerCase()==='обложка');}
function isMarkerTag(categories){return categories.some(x=>String(x).trim().toLowerCase()==='метка');}
function isEightToThree(width,height){return Number.isFinite(width)&&Number.isFinite(height)&&height>0&&Math.abs((width/height)-(8/3))<=0.001;}
async function syncGovernmentVisual(env,itemId,categories,governmentEnabled){const id=String(itemId||'').trim(),isMarker=isMarkerTag(categories),isCover=isCoverTag(categories);const govs=await env.DB.prepare(`SELECT id FROM territories WHERE status='approved' AND is_government=1`).all();if(!governmentEnabled){await env.DB.prepare(`UPDATE city_customization SET marker_item_id=NULL,updated_at=? WHERE marker_item_id=? AND territory_id IN (SELECT id FROM territories WHERE status='approved' AND is_government=1)`).bind(Date.now(),id).run();await env.DB.prepare(`UPDATE city_customization SET background_item_id=NULL,updated_at=? WHERE background_item_id=? AND territory_id IN (SELECT id FROM territories WHERE status='approved' AND is_government=1)`).bind(Date.now(),id).run();return;}for(const row of govs.results||[]){if(isMarker)await env.DB.prepare(`INSERT INTO city_customization(territory_id,marker_item_id,updated_at) VALUES(?,?,?) ON CONFLICT(territory_id) DO UPDATE SET marker_item_id=excluded.marker_item_id,updated_at=excluded.updated_at`).bind(row.id,id,Date.now()).run();if(isCover)await env.DB.prepare(`INSERT INTO city_customization(territory_id,background_item_id,updated_at) VALUES(?,?,?) ON CONFLICT(territory_id) DO UPDATE SET background_item_id=excluded.background_item_id,updated_at=excluded.updated_at`).bind(row.id,id,Date.now()).run();}if(!isMarker)await env.DB.prepare(`UPDATE city_customization SET marker_item_id=NULL,updated_at=? WHERE marker_item_id=? AND territory_id IN (SELECT id FROM territories WHERE status='approved' AND is_government=1)`).bind(Date.now(),id).run();if(!isCover)await env.DB.prepare(`UPDATE city_customization SET background_item_id=NULL,updated_at=? WHERE background_item_id=? AND territory_id IN (SELECT id FROM territories WHERE status='approved' AND is_government=1)`).bind(Date.now(),id).run();}
...
async function createCustomizationItem(request, env) {
  ...
  const isMarker=isMarkerTag(categories), isCover=isCoverTag(categories);
  ...
  if(governmentMarker&&!isMarker&&!isCover)return json({ok:false,error:'Для государственной метки или обложки визуал должен иметь тег «Метка» или «Обложка»'},400);
  if(isCover){const dim=imageDimensions(imageData);if(!dim||!isEightToThree(dim.width,dim.height))return json({ok:false,error:'Для визуала с тегом «Обложка» изображение должно иметь соотношение 8:3'},400);}
  ...
  try{
    ...
    await env.DB.prepare(`
      ...
      (id,category_id,name,price,image_data,active,created_by,created_at,categories_text,color,sort_order,
       ...
       achievement_screenshots,achievement_comments,achievement_visuals,government_marker)
      VALUES (?,?,?,?,?,1,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    `).bind(
      ...
      achievement.inspectorScore,achievement.playerScore,achievement.screenshots,achievement.comments,achievement.visuals,governmentMarker?1:0
    ).run();
    await cleanupCustomizationCategories(env);
    await syncGovernmentVisual(env,itemId,categories,governmentMarker);
    if(achievement.enabled){
    ...
    return json({ok:true,item:{id,category_id:categoryIds[0],categories_text:categories.join(', '),name,price,image_data:imageData,color,sort_order:Number(maxRow?.max_order??-1)+1,achievement_enabled:achievement.enabled?1:0,government_marker:governmentMarker?1:0}});
  }catch(error){
...
async function updateCustomizationItem(request,env,itemId){
  ...
  const governmentMarker=body?.governmentMarker===undefined?!!current.government_marker:!!body.governmentMarker;
  const isMarker=isMarkerTag(categories), isCover=isCoverTag(categories);
  if(governmentMarker&&!isMarker&&!isCover)return json({ok:false,error:'Для государственной метки или обложки визуал должен иметь тег «Метка» или «Обложка»'},400);
  ...
  await env.DB.prepare(`
    UPDATE customization_items SET category_id=?,name=?,price=?,image_data=?,categories_text=?,color=?,
      ...
      achievement_player_score=?,achievement_screenshots=?,achievement_comments=?,achievement_visuals=?,government_marker=?
    WHERE id=?
  `).bind(categoryIds[0],name,price,imageData,categories.join(', '),color,achievement.enabled?1:0,achievement.residents,
    achievement.gems,achievement.inspectorScore,achievement.playerScore,achievement.screenshots,achievement.comments,achievement.visuals,governmentMarker?1:0,id).run();
  await cleanupCustomizationCategories(env);
  await syncGovernmentVisual(env,id,categories,governmentMarker);
  if(achievement.enabled) {
    const territories=await env.DB.prepare(`SELECT id FROM territories WHERE status='approved'`).all();
    for(const t of territories.results||[]) await evaluateAchievementsForTerritory(env,t.id);
  }
...
async function deleteCustomizationItem(request,env,itemId){
  ...
  await env.DB.prepare(`UPDATE city_customization SET background_item_id=NULL,updated_at=? WHERE background_item_id=?`).bind(Date.now(),id).run();
...
async function purchaseCustomizationItem(request, env) {
  ...
  if(accountCity) await env.DB.prepare(`INSERT OR IGNORE INTO city_treasury(territory_id,balance,updated_at) VALUES(?,?,?)`).bind(accountCity.id,0,Date.now()).run();
  const item=await env.DB.prepare('SELECT id,price,government_marker FROM customization_items WHERE id=? AND active=1').bind(itemId).first(); if(!item)return json({ok:false,error:'Товар не найден'},404); if(Number(item.government_marker||0)===1)return json({ok:false,error:'Этот визуал недоступен для покупки'},403);
  ...
  // A newly obtained marker is immediately eligible as the city's default marker
  // when this user belongs to a city where no marker has been equipped yet.
  const associatedCities=[...managerCities];
  const memberCities=await env.DB.prepare(`
    SELECT t.id,t.name
    FROM city_memberships cm JOIN territories t ON t.id=cm.territory_id
    WHERE cm.telegram_id=? AND t.status='approved'
  `).bind(userId).all();
  for(const c of memberCities.results||[]) if(!associatedCities.some(x=>String(x.id)===String(c.id))) associatedCities.push(c);
  for(const c of associatedCities) await ensureFirstCityMarker(env,c.id);

  if(Number(item.price)>0){
    const attributedCity=accountCity||associatedCities[0]||null;
    if(attributedCity){await env.DB.prepare(`INSERT INTO gem_spend_log(id,territory_id,telegram_id,item_id,amount,created_at) VALUES(?,?,?,?,?,?)`).bind(crypto.randomUUID(),attributedCity.id,userId,itemId,Number(item.price),acquiredAt).run();await evaluateAchievementsForTerritory(env,attributedCity.id);}
  }
...
async function getTreasury(request, env) {
  const auth=await requireUser(request,env); if(auth.error)return auth; await ensureWorldTablesOnce(env);
...
async function treasuryDeposit(request, env) {
  const auth=await requireUser(request,env); if(auth.error)return auth; await ensureWorldTablesOnce(env); let body; try{body=await request.json()}catch{return json({ok:false,error:'Invalid JSON'},400)}
...
async function updateCityCustomization(request, env) {
  ...
  const backgroundItemId=body?.backgroundItemId?String(body.backgroundItemId):null;
  const territory=await env.DB.prepare(`SELECT id,owner_telegram_id,is_government FROM territories WHERE id=? AND status='approved'`).bind(territoryId).first();
  if(!territory)return json({ok:false,error:'City not found'},404);
  if(Number(territory.is_government||0)===1)return json({ok:false,error:'Государственной территорией нельзя управлять через оформление города'},403);
  ...
  const hasMarkerField=Object.prototype.hasOwnProperty.call(body,'markerItemId');
  const hasBackgroundField=Object.prototype.hasOwnProperty.call(body,'backgroundItemId');
  async function validateItem(itemId,tag,label){
    const item=await env.DB.prepare(`SELECT id,categories_text,government_marker FROM customization_items WHERE id=? AND active=1`).bind(itemId).first();
    if(!item)throw new Error(`${label} не найден`);
    if(Number(item.government_marker||0)===1)throw new Error('Государственный визуал нельзя экипировать на обычный город');
    const tags=String(item.categories_text||'').split(',').map(x=>x.trim().toLowerCase()).filter(Boolean);
    if(!tags.includes(tag))throw new Error(`${label} должен иметь тег «${tag}»`);
    const allowed=await env.DB.prepare(`
      ...
      LIMIT 1
    `).bind(territoryId,itemId,Number(auth.user.id),itemId,territoryId,itemId).first();
    if(!allowed)throw new Error(`${label} недоступен вашему городу`);
  }
  try{
    if(hasMarkerField){
      if(markerItemId){await validateItem(markerItemId,'метка','Метка');updates.push('marker_item_id=?');binds.push(markerItemId);}
      else {updates.push('marker_item_id=?');binds.push(null);}
    }
    if(hasBackgroundField){
      if(backgroundItemId){await validateItem(backgroundItemId,'обложка','Обложка');updates.push('background_item_id=?');binds.push(backgroundItemId);}
      else {updates.push('background_item_id=?');binds.push(null);}
    }
    if(!updates.length)return json({ok:false,error:'Нужно выбрать метку или обложку'},400);
    const columns=['territory_id',...updates.map(x=>x.replace(/\?$/,'').replace(/=$/,'')).map(x=>x.replace(/\?$/,'').trim())];
    // Build the UPSERT from explicit column names. The previous implementation
    // tried to derive columns by slicing `marker_item_id=?`, which produced
    // `marker_item_id=` and caused SQLite: near "=": syntax error.
    const values=[territoryId,...binds,Date.now()];
    const insertColumns=['territory_id',...columns.slice(1),'updated_at'];
    const assignments=columns.slice(1).map(col=>`${col}=excluded.${col}`).concat('updated_at=excluded.updated_at');
    await env.DB.prepare(`INSERT INTO city_customization(${insertColumns.join(',')}) VALUES(${insertColumns.map(()=>'?').join(',')}) ON CONFLICT(territory_id) DO UPDATE SET ${assignments.join(',')}`).bind(...values).run();
    return json({ok:true,markerItemId,backgroundItemId});
  }catch(error){return json({ok:false,error:error?.message||'Не удалось обновить оформление'},400)}
}
...
async function reconcileCityCustomizationAfterMemberLeaves(env, territoryId, telegramId) {
  const tid=String(territoryId||'').trim();
  const uid=Number(telegramId||0);
  if(!tid || !Number.isSafeInteger(uid) || uid<=0) return;
  const current=await env.DB.prepare(`SELECT marker_item_id,background_item_id FROM city_customization WHERE territory_id=?`).bind(tid).first();
  if(!current) {
    await ensureFirstCityMarker(env,tid);
    return;
  }

  const managers=await getTerritoryManagerRows(env,tid);
  const managerIds=managers.map(m=>Number(m.telegram_id)).filter(Boolean);
  const members=await env.DB.prepare(`SELECT telegram_id FROM city_memberships WHERE territory_id=?`).bind(tid).all();
  const residentIds=[...managerIds,...(members.results||[]).map(r=>Number(r.telegram_id)).filter(Boolean)]
    .filter((x,i,a)=>a.indexOf(x)===i);
  const ph=residentIds.length?residentIds.map(()=>'?').join(','):'NULL';

  async function cityHasItem(itemId){
    if(!itemId) return false;
    const row=await env.DB.prepare(`
      SELECT 1 FROM city_items
      WHERE territory_id=? AND item_id=?
      UNION
      SELECT 1 FROM user_items
      WHERE telegram_id IN (${ph}) AND item_id=?
      LIMIT 1
    `).bind(tid,...residentIds,String(itemId)).first();
    return !!row;
  }

  let markerId=current.marker_item_id||null;
  let backgroundId=current.background_item_id||null;
  let changed=false;

  if(markerId && !await cityHasItem(markerId)){ markerId=null; changed=true; }
  if(backgroundId && !await cityHasItem(backgroundId)){ backgroundId=null; changed=true; }

  if(changed){
    await env.DB.prepare(`
      INSERT INTO city_customization(territory_id,marker_item_id,background_item_id,updated_at)
      VALUES(?,?,?,?)
      ON CONFLICT(territory_id) DO UPDATE SET
        marker_item_id=excluded.marker_item_id,
        background_item_id=excluded.background_item_id,
        updated_at=excluded.updated_at
    `).bind(tid,markerId,backgroundId,Date.now()).run();
  }

  // A city must always have its first available marker equipped.
  if(!markerId) await ensureFirstCityMarker(env,tid);
}
...
async function removeResident(request, env) {
  ...
  // Purchased visuals stay in the user's personal inventory, but after leaving
  // they are no longer available to the city. If one was equipped on the city,
  // remove it; the marker falls back to the city's first available marker and
  // a removed cover naturally becomes the default gradient in the frontend.
  await reconcileCityCustomizationAfterMemberLeaves(env,territoryId,targetId);

  try{await telegram(env,'sendMessage',{chat_id:targetId,text:`🚪 Вы исключены из города\n\n🏙 ${escapeHtml((await env.DB.prepare('SELECT name FROM territories WHERE id=?').bind(territoryId).first())?.name||'Город')}`,parse_mode:'HTML'})}catch{} return json({ok:true});
}
...
async function handleResidentInviteCallback(env, callbackQuery) {
  const [action, inviteId]=String(callbackQuery.data||'').split(':'); if(!inviteId||!['resident_accept','resident_reject'].includes(action))return json({ok:true}); await ensureWorldTablesOnce(env); const actorId=Number(callbackQuery.from?.id||0);
...
async function getCityCustomizationData(request, env, url) {
  ...
  if(!(await isTerritoryManager(env,territoryId,auth.user.id)))return json({ok:false,error:'Недостаточно прав'},403);
  const city=await env.DB.prepare('SELECT marker_item_id,background_item_id FROM city_customization WHERE territory_id=?').bind(territoryId).first();
  ...
  return json({ok:true,markerItemId:city?.marker_item_id||null,backgroundItemId:city?.background_item_id||null,availableMarkers:[...(cityItems.results||[]),...(userItems.results||[])],recruitmentUsername});
}

async function getTerritories(request, env) {
  try {
    await ensureRecruitmentTableOnce(env);
    await ensureWorldTablesOnce(env);
    ...
    const result = await env.DB.prepare(`
      SELECT t.*,
        ...
        cc.background_item_id,
        bi.name AS background_name,
        bi.image_data AS background_image,
      ...
      LEFT JOIN customization_items bi ON bi.id=cc.background_item_id
    ...
    const territories=(result.results||[]).map(row=>{
      ...
      return {...row,x,y,vox_x:parsed?.x??null,vox_z:parsed?.z??null,curator:weightedRows(inspectorRows,2),community:weightedRows(playerRows,5),votes:playerRows.length,my_rating:myRating,owner:isGovernment?'':(row.owner_mc_nickname||'Ник не указан'),owner_mc_nickname:isGovernment?null:(row.owner_mc_nickname||null),founder_mc_nickname:isGovernment?null:(row.owner_mc_nickname||null),managers,manager_telegram_ids:managerIds,is_manager:isManagerViewer,residents:isGovernment?[]:ms.map(m=>m.mc_nickname).filter(Boolean),resident_telegram_ids:isGovernment?[]:ms.map(m=>Number(m.telegram_id)),resident_count:isGovernment?0:ms.length+managers.length,recruitment_contact_username:isGovernment?null:(row.recruitment_contact_username||row.owner_telegram_username||null),reviews:commentsByCity[row.id]||[],screenshots:screensByCity[row.id]||[],marker_url:row.marker_image||null,cover_url:row.background_image||null,is_government:isGovernment};
    });
...
Чтобы дополнить или обновить карту Vox, создайте, полностью заполните карту без локатора 2-го уровня и отправьте на почтовый адрес "Vox" (карта будет отсканирована и отправлена адресанту, возможно с вознаграждением).`;

async function getFaq(env) {
  try {
    await ensureMapSettingsTable(env);
    const row = await env.DB.prepare("SELECT value FROM app_settings WHERE key = 'faq_text'").first();
    const legacyFaq = `Voxygen — карта городов и их жителей.
...
Визуал: метки и другие элементы оформления города.`;
    const text = !row?.value || row.value.trim() === legacyFaq.trim() ? DEFAULT_FAQ_TEXT : row.value;
...
async function uploadScreenshot(request, env) {
  const auth=await requireUser(request,env);if(auth.error)return auth;
  await ensureWorldTablesOnce(env);
  const user=auth.user;let form;try{form=await request.formData()}catch(error){return json({ok:false,error:'Не удалось прочитать изображение',details:String(error)},400)}
  ...
  const territory=await env.DB.prepare(`SELECT id,name,owner_telegram_id,owner_telegram_username,is_government FROM territories WHERE id=? AND status='approved'`).bind(territoryId).first();if(!territory)return json({ok:false,error:'City not found'},404);
  const role=await getRoleByTelegramId(env,user.id);
  const isGovernment=Number(territory.is_government||0)===1;
  ...
  const username=normalizeUsername(user.username)||`id${user.id}`,token=String(env.BOT_TOKEN||'').trim(),screenshotId=crypto.randomUUID(),status=(isGovernment||role==='inspector'||role==='main_inspector')?'approved':'pending';
  // Government screenshots do not need Telegram moderation. Store the prepared file
  // directly so any authorized user can add them even though the territory has no owner.
  if(isGovernment){
    try{
      const bytes=new Uint8Array(await file.arrayBuffer());
      if(bytes.byteLength>900000)return json({ok:false,error:'Скриншот слишком большой после сжатия'},413);
      let binary='';
      const chunk=0x8000;
      for(let i=0;i<bytes.length;i+=chunk)binary+=String.fromCharCode(...bytes.subarray(i,i+chunk));
      const imageData=btoa(binary);
      await env.DB.prepare(`INSERT INTO screenshots(id,territory_id,telegram_id,telegram_username,file_id,file_unique_id,status,image_data,created_at,moderated_by,moderated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)`).bind(screenshotId,territoryId,Number(user.id),username,'',null,'approved',imageData,Date.now(),Number(user.id),Date.now()).run();
      return json({ok:true,screenshot:{id:screenshotId,status:'approved',url:`/api/screenshots/${screenshotId}`}});
    }catch(error){
      console.error('Government screenshot direct upload error:',error);
      return json({ok:false,error:'Не удалось сохранить скриншот государственной территории'},500);
    }
  }
  let tgData=null;
  if(status==='pending'){
...
async function saveRecruitment(request, env) {
  await ensureRecruitmentTableOnce(env); const auth=await requireUser(request,env); if(auth.error)return auth; let body; try{body=await request.json()}catch{return json({ok:false,error:'Invalid JSON'},400)}
...
async function finalizeTerritoryApproval(env, chatId, id, name, owner, x, z, isGovernment=false) {
  ...
  if (isGovernment) {
    const marker=await env.DB.prepare(`SELECT id FROM customization_items WHERE active=1 AND government_marker=1 AND instr(',' || replace(lower(COALESCE(categories_text,'')),' ','') || ',', ',метка,') > 0 ORDER BY created_at DESC LIMIT 1`).first();
    const cover=await env.DB.prepare(`SELECT id FROM customization_items WHERE active=1 AND government_marker=1 AND instr(',' || replace(lower(COALESCE(categories_text,'')),' ','') || ',', ',обложка,') > 0 ORDER BY created_at DESC LIMIT 1`).first();
    if(marker) await env.DB.prepare(`UPDATE city_customization SET marker_item_id=?,updated_at=? WHERE territory_id=?`).bind(marker.id,Date.now(),id).run();
    if(cover) await env.DB.prepare(`UPDATE city_customization SET background_item_id=?,updated_at=? WHERE territory_id=?`).bind(cover.id,Date.now(),id).run();
  }
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/" && request.method === "GET") {
      ...
      // Preserve CORS even for an unexpected Worker runtime exception.
      // Otherwise browser/Telegram often exposes only "Failed to fetch".
