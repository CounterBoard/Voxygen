...
async function createTerritory(request, env) {
  ...
  try {
    ...
    const text =
      ...
      `📍 <b>Координаты:</b> ${escapeHtml(coords)}\n` +
...
async function startTerritoryApproval(env, chatId, territoryId) {
  ...
  await telegram(env, "sendMessage", {
    ...
    parse_mode: "HTML",
    reply_markup: { force_reply: true, input_field_placeholder: "Например: Айсбург" },
  });
}
...
async function handleTerritoryApprovalStep(env, message) {
  ...
  if (session.step === 1) {
    ...
    await telegram(env, 'sendMessage', { chat_id: chatId, text: '👤 <b>Владелец города</b>\n\nВведите username владельца, например <code>@Steve</code>.', parse_mode: 'HTML', reply_markup: { force_reply: true, input_field_placeholder: '@Steve' } });
    return true;
  }
  if (session.step === 2) {
    ...
    await telegram(env, 'sendMessage', { chat_id: chatId, text: '📍 <b>Координата X</b>\n\nВведите число от −2000 до 2000.', parse_mode: 'HTML', reply_markup: { force_reply: true, input_field_placeholder: '420' } });
    return true;
  }
  if (session.step === 3) {
    const x = Number(text.replace(',', '.'));
    if (!Number.isFinite(x) || x < -2000 || x > 2000) {
      await telegram(env, 'sendMessage', { chat_id: chatId, text: '⚠️ X должен быть числом от −2000 до 2000.', reply_markup: { force_reply: true, input_field_placeholder: '420' } });
      return true;
    }
    ...
    await telegram(env, 'sendMessage', { chat_id: chatId, text: '📍 <b>Координата Z</b>\n\nВведите число от −2000 до 2000.', parse_mode: 'HTML', reply_markup: { force_reply: true, input_field_placeholder: '-180' } });
    return true;
  }
  if (session.step === 4) {
    const z = Number(text.replace(',', '.'));
    if (!Number.isFinite(z) || z < -2000 || z > 2000) {
      await telegram(env, 'sendMessage', { chat_id: chatId, text: '⚠️ Z должен быть числом от −2000 до 2000.', reply_markup: { force_reply: true, input_field_placeholder: '-180' } });
      return true;
    }
    const x = session.x;
...
async function webhook(request, env) {
  ...
  try {
    const territory = await env.DB.prepare(`SELECT id, name, owner_input, coords FROM territories WHERE id = ? AND status = ?`).bind(territoryId, "pending").first();
    if (!territory) {
      await telegram(env, "answerCallbackQuery", { callback_query_id: callbackQuery.id, text: "Заявка уже обработана", show_alert: true });
      return json({ ok: true, territoryId, status: "already_processed" });
    }
    await startTerritoryApproval(env, callbackQuery.message.chat.id, territoryId);
    await telegram(env, "answerCallbackQuery", { callback_query_id: callbackQuery.id, text: "Начинаем заполнение" });
    try { await telegram(env, "editMessageReplyMarkup", { chat_id: callbackQuery.message.chat.id, message_id: callbackQuery.message.message_id, reply_markup: { inline_keyboard: [] } }); } catch {}
