...
async function addInspector(request, env) {
  ...
  if (String(telegramId) === String(env.INSPECTOR_CHAT_ID || "").trim()) {
...
async function handleTerritoryApprovalStep(env, message) {
  ...
  if (!parsed) {
    await telegram(env, "sendMessage", {
      chat_id: chatId,
      text: "⚠️ Неверный формат. Отправьте одним сообщением ровно 3 строки:\nVoxelor\n@CounterBoardaun\n900 -1200",
    });
    return true;
  }

  const approved = await finalizeTerritoryApproval(
    env, chatId, session.territory_id, parsed.name, parsed.owner, parsed.x, parsed.z
  );

  if (approved) {
    await env.DB.prepare(
      "DELETE FROM territory_approval_sessions WHERE territory_id = ?"
    ).bind(session.territory_id).run();
  }
