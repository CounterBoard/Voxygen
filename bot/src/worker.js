...
async function ensureDefaultCityVisuals(env, territoryId) {
  const defaults = ['default-marker-black','default-marker-brown','default-marker-white'];
...
async function getCityCustomizationData(request, env, url) {
  ...
  const [cityItems,userItems]=await Promise.all([
    env.DB.prepare(`SELECT ci.item_id,i.name,i.image_data,c.name AS category_name,ci.source FROM city_items ci JOIN customization_items i ON i.id=ci.item_id JOIN customization_categories c ON c.id=i.category_id WHERE ci.territory_id=? AND lower(c.name)='метка'`).bind(territoryId).all(),
    env.DB.prepare(`SELECT ui.telegram_id,ui.item_id,i.name,i.image_data,c.name AS category_name,ui.source FROM user_items ui JOIN customization_items i ON i.id=ui.item_id JOIN customization_categories c ON c.id=i.category_id WHERE ui.telegram_id IN (${ph}) AND lower(c.name)='метка'`).bind(...ids).all(),
  ]);
...
async function getTerritories(env) {
  try{
    ...
    const [result,members,comments,screens]=await Promise.all([
      env.DB.prepare(`SELECT t.*,r.description AS recruitment_description,COALESCE(r.enabled,0) AS recruitment_enabled,cc.marker_item_id,mi.name AS marker_name,mi.image_data AS marker_image FROM territories t LEFT JOIN recruitment r ON r.territory_id=t.id LEFT JOIN city_customization cc ON cc.territory_id=t.id LEFT JOIN customization_items mi ON mi.id=cc.marker_item_id WHERE t.status='approved' ORDER BY t.created_at DESC`).all(),
      ...
      env.DB.prepare(`SELECT c.id,c.territory_id,c.telegram_id,c.telegram_username,c.text,c.role,c.created_at,cr.text AS reply_text FROM comments c LEFT JOIN comment_replies cr ON cr.comment_id=c.id ORDER BY CASE WHEN c.role IN ('inspector','main_inspector') THEN 0 ELSE 1 END,c.created_at DESC`).all(),
    ...
    const byCity={},commentsByCity={},screensByCity={};for(const m of members.results||[])(byCity[m.territory_id]||=[]).push(m);for(const c of comments.results||[])(commentsByCity[c.territory_id]||=[]).push({...c,author:c.telegram_username?`@${String(c.telegram_username).replace(/^@/,'')}`:`id${c.telegram_id}`,reply:c.reply_text||null,likes:0,dislikes:0});for(const s of screens.results||[])(screensByCity[s.territory_id]||=[]).push({...s,author:s.telegram_username?`@${String(s.telegram_username).replace(/^@/,'')}`:`id${s.telegram_id}`,url:`/api/screenshots/${s.id}`});
    const territories=(result.results||[]).map(row=>{const parsed=parseRequestCoords(row.coords),ms=byCity[row.id]||[];let x=null,y=null;if(parsed){x=((parsed.x+2000)/4000)*100;y=((parsed.z+2000)/4000)*100}return {...row,x,y,vox_x:parsed?.x??null,vox_z:parsed?.z??null,owner:normalizeUsername(row.owner_telegram_username||row.owner_input)||row.owner_input||'Неизвестно',residents:ms.map(m=>m.mc_nickname).filter(Boolean),resident_telegram_ids:ms.map(m=>Number(m.telegram_id)),resident_count:ms.length,reviews:commentsByCity[row.id]||[],screenshots:screensByCity[row.id]||[],marker_url:row.marker_image||null}});
...
async function getComments(request, env, url) {
  ...
  const rows = await env.DB.prepare(`
    SELECT c.id,c.telegram_id,c.telegram_username,c.text,c.role,c.created_at,cr.text AS reply_text
    FROM comments c LEFT JOIN comment_replies cr ON cr.comment_id=c.id
...
async function uploadScreenshot(request, env) {
  const auth=await requireUser(request,env);if(auth.error)return auth;const user=auth.user;const form=await request.formData();
  ...
  const role=await getRoleByTelegramId(env,user.id);if(role==='user'&&!territory.owner_telegram_id)return json({ok:false,error:'У города пока не привязан Telegram основателя/мэра'},409);
  const username=normalizeUsername(user.username)||`id${user.id}`,token=String(env.BOT_TOKEN||'').trim(),screenshotId=crypto.randomUUID(),status=(role==='inspector'||role==='main_inspector')?'approved':'pending',targetChat=status==='pending'?territory.owner_telegram_id:user.id;
  ...
  const tgResponse=await fetch(`${TELEGRAM_API}/bot${token}/sendPhoto`,{method:'POST',body:tgForm});const tgData=await tgResponse.json();if(!tgResponse.ok||!tgData.ok)return json({ok:false,error:'Telegram error',telegram:tgData},502);
